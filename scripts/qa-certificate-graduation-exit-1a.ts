...
async function main() {
  ...
  await denied(() => processCertificateBatchRow(db,interrupted.batch.id,2,issue), "Missing Georgia interrupts PDF without issuing");
  delete process.env.CERTIFICATE_GEORGIA_BOLD_PATH;
  ...
  check(backup.metadata.backupVersion === 46, "Durable extension uses backup v46");
  ...
  legacy.metadata.backupVersion = 45; check(!!parseAndValidateBackup(JSON.stringify(legacy)), "v45 backups remain accepted");
  const restores: any[] = [];
