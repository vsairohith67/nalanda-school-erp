...
$process=Start-Process -FilePath "pnpm.cmd" -ArgumentList @("exec","next","dev","--hostname","127.0.0.1","--port","$port") -WorkingDirectory $projectRoot -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath -WindowStyle Hidden -PassThru
...
Write-Output "STARTED port=$port pid=$($listener.OwningProcess) copiedDatabase=$databasePath"
