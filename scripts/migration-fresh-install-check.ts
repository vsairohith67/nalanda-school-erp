...
export async function runMigrationFreshInstallCheck() {
  ...
  try {
    ...
    db.close();
    if (users !== 4 || aliases !== 4 || students !== 8 || migrations !== 23 || operationalChecks !== 13) throw new Error("SYNTHETIC_BOOTSTRAP_COUNTS_INVALID");
    success = true;
    ...
    return { databasePath, schema, users, aliases, students, migrations, operationalChecks };
  } finally {
