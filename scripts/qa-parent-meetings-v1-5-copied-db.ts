...
async function main() {
  ...
  try {
    ...
    await denied(() => scheduleParentMeeting(client, principal, conflict.publicKey, { expectedRowVersion: conflict.rowVersion, scheduledStartAt: "2026-09-10T10:10:00+05:30", durationMinutes: 30, mode: "PHONE", locationReference: "", primaryStaffHandle: base.staff.get("TEACHER").iamPublicKey, participantStaffHandles: [base.staff.get("TEACHER").iamPublicKey] }), `${prefix}_STAFF_CONFLICT_ACCEPTED`);
    await denied(() => scheduleParentMeeting(client, principal, conflict.publicKey, { expectedRowVersion: conflict.rowVersion, scheduledStartAt: "2026-09-10T10:10:00+05:30", durationMinutes: 30, mode: "IN_PERSON", locationReference: "Principal Office", primaryStaffHandle: base.staff.get("TEACHER_B").iamPublicKey, participantStaffHandles: [base.staff.get("TEACHER_B").iamPublicKey] }), `${prefix}_LOCATION_CONFLICT_ACCEPTED`);
    ...
    await denied(() => scheduleParentMeeting(client, principal, guardianConflict.publicKey, { expectedRowVersion: guardianConflict.rowVersion, scheduledStartAt: "2026-09-10T10:10:00+05:30", durationMinutes: 30, mode: "PHONE", primaryStaffHandle: base.staff.get("TEACHER_B").iamPublicKey, participantStaffHandles: [base.staff.get("TEACHER_B").iamPublicKey] }), `${prefix}_GUARDIAN_CONFLICT_ACCEPTED`);
