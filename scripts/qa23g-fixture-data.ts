...
export async function seedReport23GFixtures(client: PrismaClient, options: { suite: string; browserPassword?: string; academicYear?: string; className?: string }) {
  const prefix = `${options.suite}-${randomBytes(4).toString("hex")}`.toUpperCase();
  const academicYear = options.academicYear ?? REPORT23G_YEAR;
  const className = options.className ?? REPORT23G_CLASS;
  const calendarYear = Number(academicYear.slice(0, 4));
  ...
  const classSections = await Promise.all(sections.map((section) => client.timetableClassSection.create({ data: {
    ...
    displayName: `${className}-${section}`, groupName: prefix, isActive: true
  } })));
  ...
  const timetableAssignment = await client.timetableAssignment.create({ data: { academicYear, classSectionId: classSections[0].id, subjectId: math.id, teacherId: timetableTeacher.id, periodsPerWeek: 5 } });
  ...
  for (const [sectionIndex, count] of [6, 2].entries()) {
    ...
    for (let index = 0; index < count; index++) {
      const student = await client.student.create({ data: {
        ...
        className, section, rollNo: String(index + 1), phone1: "0000000000"
      } });
      await client.academicYearEnrollment.create({ data: { studentId: student.id, academicYear, className, section, rollNo: String(index + 1), status: "ACTIVE" } });
  ...
  const template = await client.reportCardTemplate.create({ data: {
    templateCode: `${prefix}-REPORT`, name: `${prefix} governed report`, reportType: "MARK_BASED", academicYear,
    className, status: "ACTIVE", templateDefinitionJson: JSON.stringify(DEFAULT_MARK_TEMPLATE), printSettingsJson: "{}", createdByUserId: principal.user.id
  } });
  const examSpecifications = [
    { code: `${prefix}-REV`, name: `Class ${className} Revision`, maximum: 100, mode: "RAW_SUM", formula: "EXAM_RESULT_V1", start: `${calendarYear}-06-01`, end: `${calendarYear}-06-05` },
    { code: `${prefix}-PRE`, name: `Class ${className} Preboard`, maximum: 80, mode: "RAW_SUM", formula: "EXAM_RESULT_V1", start: `${calendarYear}-07-01`, end: `${calendarYear}-07-05` },
    { code: `${prefix}-WGT`, name: `Class ${className} Weighted Review`, maximum: 100, mode: "WEIGHTED_NORMALIZED", formula: "EXAM_RESULT_WEIGHTED_V1", start: `${calendarYear}-07-15`, end: `${calendarYear}-07-20` }
  ] as const;
  const resultSnapshots: any[] = [], reportCardVersions: any[] = [];
  for (const [examIndex, specification] of examSpecifications.entries()) {
    const examination = await client.examination.create({ data: { examCode: specification.code, academicYear, name: specification.name, examType: examIndex === 0 ? "REVISION" : "PREBOARD", startDate: new Date(specification.start), endDate: new Date(specification.end), status: "ACTIVE", version: 1, createdByUserId: principal.user.id, activatedByUserId: principal.user.id, activatedAt: new Date(specification.start) } });
    for (const [sectionIndex, classSection] of classSections.entries()) {
      const section = sections[sectionIndex];
      const scope = await client.examinationClassScope.create({ data: { examinationId: examination.id, academicYear, className, section, timetableClassSectionId: classSection.id, status: "ACTIVE", createdByUserId: principal.user.id } });
      const paper = await client.examSubjectPaper.create({ data: { examinationId: examination.id, classScopeId: scope.id, academicYear, className, section, timetableSubjectId: math.id, subjectNameSnapshot: math.name, paperCode: "MAT", paperName: "Mathematics", displayOrder: 1, status: "ACTIVE", createdByUserId: principal.user.id } });
      const scheme = await client.examinationSchemeVersion.create({ data: { examinationId: examination.id, classScopeId: scope.id, academicYear, className, section, scopeKey: "MAT", subjectPaperId: paper.id, versionNumber: 1, calculationMode: specification.mode, roundingPolicyVersion: "RC05_V1_DECIMAL6_HALF_UP2", markDecimalPlaces: 2, status: "ACTIVE", frozenAt: new Date(specification.start), createdByUserId: principal.user.id, activatedByUserId: principal.user.id, activatedAt: new Date(specification.start) } });
      ...
      const batch = await client.reportCardBatch.create({ data: { batchNumber: `${prefix}-${examIndex}-${section}`, academicYear, reportType: "MARK_BASED", templateId: template.id, className, section, title: specification.name, reportingPeriod: `${specification.start} to ${specification.end}`, status: "ISSUED", templateSnapshotJson: JSON.stringify(templateSnapshot), createdByUserId: principal.user.id, issuedByUserId: principal.user.id, issuedAt: new Date(`${specification.end}T12:00:00Z`) } });
      ...
      for (const [studentIndex, student] of sectionStudents.entries()) {
        ...
        const card = await client.studentReportCard.create({ data: { reportCardNumber: `${prefix}-${examIndex}-${section}-${studentIndex + 1}`, batchId: batch.id, studentId: student.id, academicYear, className, section, reportType: "MARK_BASED", status: "ISSUED", currentVersionNumber: 1, draftDataJson: JSON.stringify({ publicationSchemaVersion: 3, previewFingerprint: sha(snapshot.id) }), finalGrade: percentage >= 75 ? "A" : percentage >= 40 ? "B" : "D", createdByUserId: principal.user.id, issuedByUserId: principal.user.id, issuedAt: new Date(`${specification.end}T12:00:00Z`) } });
        const published = publishedSnapshot({ prefix, examination, specification, student, snapshot, scheme, state, obtained, percentage, maximum: specification.maximum, reportCardNumber: card.reportCardNumber, scienceName: science.name, academicYear, className });
  ...
  return { prefix, academicYear, className, sections, students, resultSnapshots, reportCardVersions, users: { principal, director, teacher, parent, viewer, accountant, student: studentUser }, examCodes: examSpecifications.map((row) => row.code) };
}
...
function publishedSnapshot(input: any): PublishedReportSnapshot {
  ...
  return {
    ...
    issueDate: `${input.specification.end}T12:00:00.000Z`, title: input.examination.name, reportingPeriod: `${input.specification.start} to ${input.specification.end}`, academicYear: input.academicYear,
    school: { name: "Nalanda Public School", address: "Synthetic copied-database fixture", city: "Hyderabad", phone: null, logoPath: null },
    student: { name: input.student.studentName, admissionNumber: input.student.admissionNo, rollNumber: input.student.rollNo, className: input.className, section: input.student.section, dateOfBirth: null },
    ...
    signatures: [{ role: "PRINCIPAL", label: "Principal" }], template: { code: `${input.prefix}-REPORT`, name: `Governed Class ${input.className} report`, version: 1, bindingVersion: 1, definition: {}, printSettings: { orientation: "PORTRAIT", pageSize: "A4", minimumFontSizePt: 10, marginMm: 12 } },
