...
export async function runMigrationBackupRestoreCheck() {
  ...
  const backupPath = assertIsolatedDatabasePath(path.join(QA_ROOT, "restore", `DEVOPS1B-v46-${process.pid}.backup.json`));
  let success = false;
  try {
    ...
    if (validated.metadata.backupVersion !== 46) throw new Error("BACKUP_VERSION_CHANGED");
    ...
    success = true;
    console.log(`Backup/restore passed: version=46 arrays=${arrayEntries.length} students=${firstCounts.students} payments=${firstCounts.payments}`);
    ...
    return { version: 46, arrays: arrayEntries.length, firstCounts, firstBusiness };
  } finally {
    if (success) {
