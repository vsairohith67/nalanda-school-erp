...
export async function runMigrationBackupRestoreCheck() {
  ...
  const backupPath = assertIsolatedDatabasePath(path.join(QA_ROOT, "restore", `DEVOPS1B-v48-${process.pid}.backup.json`));
  let success = false;
  try {
    ...
    if (validated.metadata.backupVersion !== 48) throw new Error("BACKUP_VERSION_CHANGED");
    ...
    success = true;
    console.log(`Backup/restore passed: version=48 arrays=${arrayEntries.length} students=${firstCounts.students} payments=${firstCounts.payments}`);
    ...
    return { version: 48, arrays: arrayEntries.length, firstCounts, firstBusiness };
  } finally {
    if (success) {
