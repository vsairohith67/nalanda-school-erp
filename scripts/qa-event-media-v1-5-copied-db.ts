...
async function main() {
  ...
  try {
    ...
    invariant(logical.metadata.backupVersion === 45 && logical.eventMediaAlbums.length === 2 && !JSON.stringify(logical).includes(exifOriginal.toString("base64")), `${prefix}_LOGICAL_BACKUP_INVALID`);
  ...
  const evidence = { result: `${prefix}_COPIED_DB_QA_PASSED`, operationalSha256: operationalBefore[0]?.hash, migration: "20260822113000_event_media_v1_5_foundation", backupVersion: 45, publicPublishing: "DEFAULT_OFF", syntheticImagesOnly: true, kept: keep, ...(keep ? { runtimePath } : {}) };
