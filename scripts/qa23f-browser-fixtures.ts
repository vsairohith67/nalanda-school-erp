...
const ROOT = path.join(WORKSPACE, "tmp", "class23f-browser");
const DATABASE = path.join(ROOT, "class23f-browser.db");
...
const YEAR = "2026-27";
const CLASS_NAME = "CLASS23F-VII";
const SECTION = "A";
const PREFIX = "CLASS23F-BROWSER";
const PORT = 3221;

function databaseUrl(file: string) { return `file:${file.replaceAll("\\", "/")}`; }
function assertRoot() {
  const resolved = path.resolve(ROOT);
  if (resolved !== path.join(path.resolve(WORKSPACE), "tmp", "class23f-browser")) throw new Error("CLASS23F_BROWSER_CLEANUP_SCOPE_REFUSED");
  return resolved;
}
...
async function createUser(client: PrismaClient, input: { username: string; name: string; role: Role; password: string; guardianId?: string }) {
  ...
  const assignment = await client.userRoleAssignment.create({ data: { publicKey: randomUUID(), userId: user.id, role: input.role, reason: "CLASS23F isolated Browser fixture", assignedByUserId: user.id, activeKey: `${user.id}:${input.role}` } });
...
async function setup() {
  ...
  try {
    ...
    for (const [index, name] of ["Aarav", "Diya", "Kabir"].entries()) {
      const student = await client.student.create({ data: { admissionNo: `${PREFIX}-${index + 1}`, studentName: `${name} CLASS23F`, fatherName: "Synthetic Browser Parent", className: CLASS_NAME, section: SECTION, rollNo: String(index + 1), phone1: "0000000000" } });
    ...
    const principal = await createUser(client, { username: "class23f-browser-principal", name: `${PREFIX} Principal`, role: "PRINCIPAL", password });
    const teacher = await createUser(client, { username: "class23f-browser-teacher", name: `${PREFIX} Teacher`, role: "TEACHER", password });
    const parent = await createUser(client, { username: "class23f-browser-parent", name: `${PREFIX} Parent`, role: "PARENT", guardianId: guardian.id, password });
    const studentUser = await createUser(client, { username: "class23f-browser-student", name: `${PREFIX} Student`, role: "STUDENT", password });
    ...
    const timetableTeacher = await client.timetableTeacher.create({ data: { name: teacher.user.name, shortName: "C23F-T", isActive: true, maxPeriodsPerWeek: 30 } });
    await client.staffMember.create({ data: { iamPublicKey: randomUUID(), staffCode: "CLASS23F-BROWSER-T", fullName: teacher.user.name, designation: "Teacher", status: "ACTIVE", userId: teacher.user.id, timetableTeacherId: timetableTeacher.id } });
    ...
    published = await publishClassworkVersion(client, published.item.publicKey, { expectedVersion: published.item.rowVersion, requestKey: "CLASS23F_BROWSER_PUBLISH_0001" }, teacherActor);
    ...
    submission = await submitClasswork(client, published.item.publicKey, { expectedVersion: submission.submission.rowVersion, requestKey: "CLASS23F_BROWSER_SUBMIT_0001" }, learnerContext);
    ...
    console.log(JSON.stringify({ result: "CLASS23F_BROWSER_FIXTURES_READY", copiedDatabase: true, port: PORT, teachers: 1, parents: 1, students: 3, studentContext: true, published: 1, returned: 1, privateDrafts: 1 }));
...
function cleanup() {
  const root = assertRoot();
  if (existsSync(root)) rmSync(root, { recursive: true, force: true });
  console.log(JSON.stringify({ result: "CLASS23F_BROWSER_FIXTURES_REMOVED", exists: existsSync(root) }));
}
