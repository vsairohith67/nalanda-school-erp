...
const ROOT = path.join(WORKSPACE, "tmp", "parent23d-browser");
const DATABASE = path.join(ROOT, "parent23d-browser.db");
const CREDENTIALS = path.join(ROOT, "credentials.json");
const RUNTIME_ENV = path.join(ROOT, "runtime-env.json");
const ACADEMIC_YEAR = "2026-27";
const REASON = "PARENT23D copied production Browser QA evidence";

function databaseUrl(file: string) { return `file:${file.replaceAll("\\", "/")}`; }

function assertRoot() {
  const resolved = path.resolve(ROOT);
  if (resolved !== path.join(path.resolve(WORKSPACE, "tmp"), "parent23d-browser")) throw new Error("PARENT23D_BROWSER_CLEANUP_SCOPE_REFUSED");
  return resolved;
}
...
async function setup() {
  ...
  try {
    const guardians = await Promise.all([
      client.guardian.create({ data: { iamPublicKey: randomUUID(), displayName: "PARENT23D Browser One Child", primaryMobile: "9000023101" } }),
      client.guardian.create({ data: { iamPublicKey: randomUUID(), displayName: "PARENT23D Browser Two Children", primaryMobile: "9000023102" } }),
      client.guardian.create({ data: { iamPublicKey: randomUUID(), displayName: "PARENT23D Browser Teacher Parent", primaryMobile: "9000023103" } })
    ]);
    const students = await Promise.all([
      client.student.create({ data: { admissionNo: "PARENT23D-BROWSER-001", studentName: "Aarav Browser", fatherName: "Synthetic Parent", className: "P23D-V", section: "A", rollNo: "5", phone1: "0000000000" } }),
      client.student.create({ data: { admissionNo: "PARENT23D-BROWSER-002", studentName: "Diya Browser", fatherName: "Synthetic Parent", className: "P23D-V", section: "B", rollNo: "7", phone1: "0000000000" } })
    ]);
    await client.academicYearEnrollment.createMany({ data: students.map((student, index) => ({ studentId: student.id, academicYear: ACADEMIC_YEAR, className: "P23D-V", section: index === 0 ? "A" : "B", rollNo: index === 0 ? "5" : "7", status: "ACTIVE" })) });
    ...
    const principal = await createUser(client, { username: "parent23d-browser-principal", name: "PARENT23D Browser Principal", designation: "Principal", roles: ["PRINCIPAL"], password });
    const parentOne = await createUser(client, { username: "parent23d-browser-parent-one", name: "PARENT23D Browser One Child Parent", designation: "Parent", roles: ["PARENT"], guardianId: guardians[0].id, password });
    const parentMany = await createUser(client, { username: "parent23d-browser-parent-many", name: "PARENT23D Browser Multi Child Parent", designation: "Parent", roles: ["PARENT"], guardianId: guardians[1].id, password });
    const teacherParent = await createUser(client, { username: "parent23d-browser-teacher-parent", name: "PARENT23D Browser Teacher Parent", designation: "Teacher and Parent", roles: ["TEACHER", "PARENT"], guardianId: guardians[2].id, password });
    await client.staffMember.create({ data: { iamPublicKey: randomUUID(), staffCode: "P23D-BROWSER-TP", fullName: teacherParent.user.name, designation: "Teacher", userId: teacherParent.user.id } });
    ...
    for (const [index, status] of ["PRESENT", "ABSENT", "LATE", "HALF_DAY", "EXCUSED"].entries()) {
      await client.studentAttendanceSession.create({ data: {
        ...
        className: "P23D-V", section: "A", academicYear: ACADEMIC_YEAR, status: index === 4 ? "LOCKED" : "SUBMITTED",
        ...
        notes: "PARENT23D Browser private Teacher note",
        records: { create: { studentId: students[0].id, admissionNo: students[0].admissionNo, status, remarks: "PARENT23D Browser sensitive private remark" } }
      } });
    }
    await client.studentAttendanceSession.create({ data: { attendanceDate: new Date("2026-08-10T00:00:00.000Z"), className: "P23D-V", section: "A", academicYear: ACADEMIC_YEAR, status: "DRAFT", takenByUserId: principal.user.id, records: { create: { studentId: students[0].id, admissionNo: students[0].admissionNo, status: "ABSENT", remarks: "Draft must be hidden" } } } });

    const classA = await client.timetableClassSection.create({ data: { academicYear: ACADEMIC_YEAR, className: "P23D-V", section: "A", displayName: "P23D-V - A", groupName: "PARENT23D Browser", isActive: true } });
    await client.timetableClassSection.create({ data: { academicYear: ACADEMIC_YEAR, className: "P23D-V", section: "B", displayName: "P23D-V - B", groupName: "PARENT23D Browser", isActive: true } });
    const examination = await client.examination.create({ data: { examCode: "PARENT23D-BROWSER-TERM1", academicYear: ACADEMIC_YEAR, name: "September Term Examination", examType: "TERM", startDate: new Date("2026-09-01T00:00:00.000Z"), endDate: new Date("2026-09-10T00:00:00.000Z"), status: "ACTIVE", createdByUserId: principal.user.id, activatedByUserId: principal.user.id, activatedAt: new Date() } });
    const scope = await client.examinationClassScope.create({ data: { examinationId: examination.id, academicYear: ACADEMIC_YEAR, className: "P23D-V", section: "A", timetableClassSectionId: classA.id, status: "ACTIVE", createdByUserId: principal.user.id } });
    const papers: ExamSubjectPaper[] = [];
    for (const [index, name] of ["Mathematics", "Environmental Science"].entries()) {
      const subject = await client.timetableSubject.create({ data: { name: `PARENT23D Browser ${name}`, shortName: `P23D-${index + 1}`, department: "PARENT23D Browser", isActive: true } });
      papers.push(await client.examSubjectPaper.create({ data: { examinationId: examination.id, classScopeId: scope.id, academicYear: ACADEMIC_YEAR, className: "P23D-V", section: "A", timetableSubjectId: subject.id, subjectNameSnapshot: name, paperCode: `P${index + 1}`, paperName: `${name} Paper`, displayOrder: index + 1, status: "ACTIVE", createdByUserId: principal.user.id } }));
    }
    ...
    let initial = await createExaminationTimetable(client, { examinationId: examination.id, classScopeId: scope.id, idempotencyKey: "PARENT23D-BROWSER-INITIAL-0001" }, actor);
    ...
    let replacement = await createExaminationTimetable(client, { examinationId: examination.id, classScopeId: scope.id, sourceVersionId: initial.id, idempotencyKey: "PARENT23D-BROWSER-REPLACE-0001" }, actor);
    ...
    const principalDraft = await createExaminationTimetable(client, { examinationId: examination.id, classScopeId: scope.id, sourceVersionId: replacement.id, idempotencyKey: "PARENT23D-BROWSER-DRAFT-0001" }, actor);
    ...
    writeFileSync(RUNTIME_ENV, JSON.stringify({ DATABASE_URL: databaseUrl(DATABASE), SESSION_SECRET: secret, AUTH_SECRET: secret, APP_ORIGIN: "http://127.0.0.1:3218", NODE_ENV: "production", PORT: "3218" }));
    ...
    console.log(JSON.stringify({ result: "PARENT23D_BROWSER_FIXTURES_READY", copiedDatabase: true, parents: 3, principal: 1, children: 2, officialAttendanceRows: 5, currentPublishedTimetables: 1, principalDrafts: 1 }));
  } finally {
...
function cleanup() {
  const root = assertRoot();
  if (existsSync(root)) rmSync(root, { recursive: true, force: true });
  console.log(JSON.stringify({ result: "PARENT23D_BROWSER_FIXTURES_REMOVED", exists: existsSync(root) }));
}
