...
$env:NALANDA_TRUSTED_PROXY_MODE = "authenticated-edge-v1"
$env:NALANDA_REQUIRE_TRUSTED_PROXY = "true"
$env:NALANDA_PROXY_SHARED_SECRET = [Convert]::ToBase64String((1..48 | ForEach-Object { [byte](Get-Random -Minimum 0 -Maximum 256) }))
$env:NALANDA_CLIENT_IP_HEADER = "x-forwarded-for"
$env:NALANDA_LOCAL_SECURITY_REHEARSAL = "true"
$env:SECURITY_RATE_LIMIT_MODE = "single-process-rehearsal"
...
try {
  ...
  try {
    ...
    if ($backupDocument.metadata.backupVersion -ne 48 -or @($backupDocument.students).Count -ne 1 -or @($backupDocument.payments).Count -ne 0) {
  ...
  [pscustomobject]@{
    ...
    backupVersion = $backupDocument.metadata.backupVersion
