...
describe("Student item completion and restore contract", () => {
  ...
  it("uses an explicit v47 contract and accepts legacy v45 without inventing feature records", () => {
    ...
    expect(full.metadata.backupVersion).toBe(47); expect(parseAndValidateBackup(full).priorYearLiabilities).toEqual([]);
    const legacy = structuredClone(full) as any; legacy.metadata.backupVersion = 45; delete legacy.metadata.schemaContract;
    expect(parseAndValidateBackup(legacy).priorYearLiabilities).toEqual([]);
    const conflict = structuredClone(full) as any; conflict.metadata.schemaContract = "unmerged-other-branch-v47"; expect(() => parseAndValidateBackup(conflict)).toThrow("CONTRACT_MISMATCH");
