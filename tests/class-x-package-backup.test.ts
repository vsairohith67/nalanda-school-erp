...
describe("Class X package backup version 28", () => {
  it("includes all seven arrays and excludes actors/password hashes", () => { const value = backup(); expect(value.metadata.backupVersion).toBe(40); for (const key of Object.keys(classXRows())) expect((value as any)[key]).toHaveLength(1); expect(JSON.stringify(value)).not.toContain("passwordHash"); expect(JSON.stringify(value)).not.toContain("createdByUserId"); });
