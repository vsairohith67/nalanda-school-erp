...
describe("Class X package backup version 28", () => {
  it("includes all seven arrays and excludes actors/password hashes", () => { const value = backup(); expect(value.metadata.backupVersion).toBe(48); for (const key of Object.keys(classXRows())) expect((value as any)[key]).toHaveLength(1); expect(JSON.stringify(value)).not.toContain("passwordHash"); expect(JSON.stringify(value)).not.toContain("createdByUserId"); });
  it("restores empty module collections in v48 and rejects unsupported historical format (class-x-package-backup)", () => { const old: any = backup(); expect(() => parseAndValidateBackup({ ...old, metadata: { ...old.metadata, backupVersion: 27 } })).toThrow("BACKUP_SOURCE_CONTRACT_UNSUPPORTED"); for (const key of Object.keys(classXRows())) { old[key] = []; old.metadata.counts[key] = 0; } expect(parseAndValidateBackup(old).classXDocumentPackages).toEqual([]); });
