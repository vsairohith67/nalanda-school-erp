...
describe("homework backup and restore v23", () => {
  it("includes both arrays, version 37, and no password hashes", () => { const backup = fixture(); expect(backup.metadata.backupVersion).toBe(40); expect(backup.homeworkAssignments).toHaveLength(1); expect(backup.homeworkAssignmentEvents).toHaveLength(2); expect(JSON.stringify(backup)).not.toContain("passwordHash"); expect(JSON.stringify(backup)).not.toContain("secret"); });
