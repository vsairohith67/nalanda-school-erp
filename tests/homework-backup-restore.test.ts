...
describe("homework backup and restore v23", () => {
  it("includes both arrays, the current version, and no password hashes", () => { const backup = fixture(); expect(backup.metadata.backupVersion).toBe(47); expect(backup.homeworkAssignments).toHaveLength(1); expect(backup.homeworkAssignmentEvents).toHaveLength(2); expect(JSON.stringify(backup)).not.toContain("passwordHash"); expect(JSON.stringify(backup)).not.toContain("secret"); });
