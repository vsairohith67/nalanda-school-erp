...
describe("homework backup and restore v23", () => {
  it("includes both arrays, the current version, and no password hashes", () => { const backup = fixture(); expect(backup.metadata.backupVersion).toBe(48); expect(backup.homeworkAssignments).toHaveLength(1); expect(backup.homeworkAssignmentEvents).toHaveLength(2); expect(JSON.stringify(backup)).not.toContain("passwordHash"); expect(JSON.stringify(backup)).not.toContain("secret"); });
  ...
  it("restores empty module collections in v48 and rejects unsupported historical format (homework-backup-restore)", () => { const backup: any = fixture(); expect(() => parseAndValidateBackup({ ...backup, metadata: { ...backup.metadata, backupVersion: 22 } })).toThrow("BACKUP_SOURCE_CONTRACT_UNSUPPORTED"); backup.homeworkAssignments = []; backup.homeworkAssignmentEvents = []; backup.metadata.counts.homeworkAssignments = 0; backup.metadata.counts.homeworkAssignmentEvents = 0; const parsed = parseAndValidateBackup(backup); expect(parsed.homeworkAssignments).toEqual([]); expect(parsed.homeworkAssignmentEvents).toEqual([]); });
