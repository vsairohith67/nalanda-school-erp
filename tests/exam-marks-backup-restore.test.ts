...
describe("exam marks backup and restore validation", () => {
  it("uses the current version and includes all four exam arrays", () => { const backup = fixture(); expect(backup.metadata.backupVersion).toBe(48); expect(backup.examCycles).toHaveLength(1); expect(backup.examAssessments).toHaveLength(1); expect(backup.studentMarks).toHaveLength(1); expect(backup.studentMarkEvents).toHaveLength(1); });
  ...
  it("restores empty module collections in v48 and rejects unsupported historical format (exam-marks-backup-restore)", () => { const old: any = fixture(); expect(() => parseAndValidateBackup({ ...old, metadata: { ...old.metadata, backupVersion: 23 } })).toThrow("BACKUP_SOURCE_CONTRACT_UNSUPPORTED"); for (const key of ["examCycles", "examAssessments", "studentMarks", "studentMarkEvents"]) { old[key] = []; old.metadata.counts[key] = 0; } const parsed = parseAndValidateBackup(old); expect(parsed.examCycles).toEqual([]); expect(parsed.studentMarks).toEqual([]); });
