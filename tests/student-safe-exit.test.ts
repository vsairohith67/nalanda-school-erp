...
describe("SAFE-EXIT-1A governed security foundation",()=>{
  ...
  it("backs up all governed records at v40 and restores backward-compatible empty sections",()=>{
    ...
    expect(backup.metadata.backupVersion).toBe(45);expect(backup.studentDepartureRequests).toHaveLength(1);expect(backup.studentGatePasses).toHaveLength(1);expect(JSON.stringify(backup)).not.toContain("signingKey");expect(backup).not.toHaveProperty("appPushSubscriptions");
