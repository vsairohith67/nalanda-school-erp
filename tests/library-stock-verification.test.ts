...
describe("Library stock-verification foundation", () => {
  ...
  it("hard-blocks Viewer operational session access even if a stale role bundle still grants it", () => { const governance=readFileSync("lib/iam/permission-governance.ts","utf8"); const evaluator=readFileSync("lib/iam/effective-access.ts","utf8"); const nav=readFileSync("components/library-nav.tsx","utf8"); expect(governance).toContain('"VIEW_LIBRARY_STOCK_VERIFICATION"'); expect(governance).toContain('role === "VIEWER" && VIEWER_IMMUTABLE_DENIALS.has(permission)'); expect(evaluator).toContain("immutablePermissionDenial"); expect(nav).toContain('user?.role !== "VIEWER"'); });
