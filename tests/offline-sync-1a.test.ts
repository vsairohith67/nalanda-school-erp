...
describe("OFFLINE-SYNC-1A contracts", () => {
  ...
  it("backs up only durable public-device and safe ledger state in the current format", () => {
    ...
    expect(backup.metadata.backupVersion).toBe(48); expect(backup).toMatchObject({ offlineSyncDevices: [], offlineSyncMutations: [], offlineSyncEvents: [], offlineSyncConflictReviews: [] });
