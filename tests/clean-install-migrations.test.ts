...
describe("DEVOPS-1B clean-install migration repair", () => {
  ...
  it("keeps the generated baseline first and every release migration additive", () => {
  ...
  it("deploys from empty, reports clean status, matches the schema, and bootstraps synthetic data", async () => {
    ...
    expect(output).toContain("Fresh migration check passed: migrations=28 models=372 tables=372");
  ...
  it("keeps version-47 restore idempotent, accepts legacy versions, and preserves local ownership collisions", async () => {
    const output = await pnpm(["migration:restore-check"]);
    expect(output).toContain("Backup/restore passed: version=47 arrays=304");
