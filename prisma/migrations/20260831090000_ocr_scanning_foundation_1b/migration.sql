...
CREATE TABLE "OcrDocument" (
  ...
  "uploadIdentityHash" TEXT NOT NULL,
  ...
  "aggregatePixels" INTEGER NOT NULL,
  "status" TEXT NOT NULL DEFAULT 'UPLOADED',
  "languageProfile" TEXT NOT NULL DEFAULT 'ENGLISH',
  "handwritingDeclared" BOOLEAN NOT NULL DEFAULT false,
  "reviewVersion" INTEGER NOT NULL DEFAULT 1,
  "targetSnapshotVersion" TEXT NOT NULL,
  "retentionPolicyVersion" TEXT NOT NULL DEFAULT 'OCR-1B-SYNTHETIC-1',
  "sourceRetentionUntil" DATETIME NOT NULL,
  "rasterRetentionUntil" DATETIME NOT NULL,
  "rawOutputRetentionUntil" DATETIME NOT NULL,
  "candidateRetentionUntil" DATETIME NOT NULL,
  "auditRetentionUntil" DATETIME NOT NULL,
  "purgeStatus" TEXT NOT NULL DEFAULT 'NOT_REQUESTED',
  "purgeFailureCode" TEXT,
  ...
  CONSTRAINT "OcrDocument_contract" CHECK (
    "contextType" IN ('ADMISSION','STUDENT','GUARDIAN','STAFF') AND
    "sourceMediaType" IN ('image/png','image/jpeg','application/pdf') AND
    "sourceExtension" IN ('.png','.jpg','.jpeg','.pdf') AND
    "byteSize" BETWEEN 1 AND 26214400 AND "pageCount" BETWEEN 1 AND 25 AND
    "aggregatePixels" BETWEEN 1 AND 120000000 AND "reviewVersion" >= 1 AND
    "status" IN ('UPLOADED','ADMITTED','QUEUED','PROCESSING','OCR_COMPLETE','REVIEW_REQUIRED','REVIEW_IN_PROGRESS','READY_TO_SUBMIT','SUBMITTED','REJECTED','EXPIRED','PURGED','FAILED') AND
    "languageProfile" IN ('ENGLISH','HINDI','TELUGU','ENGLISH_HINDI','ENGLISH_TELUGU','ENGLISH_HINDI_TELUGU') AND
    "purgeStatus" IN ('NOT_REQUESTED','PENDING','PARTIAL_FAILURE','CONFIRMED')
  )
);
...
CREATE TABLE "OcrJob" (
  ...
  CONSTRAINT "OcrJob_contract" CHECK (
    "status" IN ('QUEUED','PROCESSING','COMPLETED','FAILED','CANCELLED','DEAD') AND
    "attemptCount" BETWEEN 0 AND "maximumAttempts" AND "maximumAttempts" BETWEEN 1 AND 5 AND
    ("outputBytes" IS NULL OR "outputBytes" BETWEEN 0 AND 52428800)
  )
);
...
CREATE TABLE "OcrPage" (
  ...
  "rasterObjectKey" TEXT NOT NULL,
  "rasterSha256" TEXT NOT NULL,
  "sourceDigest" TEXT NOT NULL,
  "sourceWidth" INTEGER NOT NULL,
  "sourceHeight" INTEGER NOT NULL,
  "sourceRotation" INTEGER NOT NULL DEFAULT 0,
  "reviewOrientation" INTEGER NOT NULL DEFAULT 0,
  "pixelCount" INTEGER NOT NULL,
  "ocrState" TEXT NOT NULL DEFAULT 'OCR_COMPLETE',
  "processingDurationMs" INTEGER NOT NULL,
  "retryPreprocessing" BOOLEAN NOT NULL DEFAULT false,
  "createdAt" DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  "updatedAt" DATETIME NOT NULL,
  CONSTRAINT "OcrPage_documentId_fkey" FOREIGN KEY ("documentId") REFERENCES "OcrDocument" ("id") ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT "OcrPage_contract" CHECK (
    "pageNumber" BETWEEN 1 AND 25 AND "sourceWidth" BETWEEN 1 AND 6000 AND "sourceHeight" BETWEEN 1 AND 6000 AND
    "pixelCount" BETWEEN 1 AND 40000000 AND "sourceRotation" IN (0,90,180,270) AND "reviewOrientation" IN (0,90,180,270) AND
    "processingDurationMs" BETWEEN 0 AND 120000 AND "ocrState" IN ('OCR_COMPLETE','REVIEW_REQUIRED','FAILED')
  )
);
...
CREATE TABLE "OcrFieldCandidate" (
  ...
  "recognitionScore" REAL,
  "scriptHint" TEXT NOT NULL,
  ...
  CONSTRAINT "OcrFieldCandidate_contract" CHECK (
    length("candidateText") <= 2000 AND length("fieldKey") BETWEEN 1 AND 80 AND "version" >= 1 AND
    ("recognitionScore" IS NULL OR "recognitionScore" BETWEEN 0.0 AND 1.0) AND
    "scriptHint" IN ('LATIN','DEVANAGARI','TELUGU','MIXED','UNKNOWN') AND
    "validationState" IN ('VALID_FORMAT','INVALID_FORMAT','AMBIGUOUS','MISSING') AND
    "reviewState" IN ('GREEN','AMBER','RED') AND
    "decision" IN ('PENDING','ACCEPTED','EDITED','REJECTED_CANDIDATE','MISSING_VALUE')
  )
);
...
CREATE TABLE "OcrSubmission" (
  ...
  "authoritativeService" TEXT NOT NULL,
  ...
  CONSTRAINT "OcrSubmission_contract" CHECK (
    "reviewVersion" >= 1 AND "status" IN ('PENDING','COMPLETED','STALE','FAILED') AND
    "authoritativeService" IN ('ADMISSIONS','STUDENTS','GUARDIANS','STAFF')
  )
);
...
CREATE TRIGGER "OcrDocument_source_immutable" BEFORE UPDATE ON "OcrDocument"
WHEN NEW."sourceObjectKey" <> OLD."sourceObjectKey" OR NEW."sourceMediaType" <> OLD."sourceMediaType" OR NEW."sourceExtension" <> OLD."sourceExtension" OR NEW."byteSize" <> OLD."byteSize" OR NEW."sourceSha256" <> OLD."sourceSha256" OR NEW."contextType" <> OLD."contextType" OR NEW."contextId" <> OLD."contextId"
BEGIN SELECT RAISE(ABORT, 'OCR_DOCUMENT_SOURCE_IMMUTABLE'); END;

CREATE TRIGGER "OcrPage_source_evidence_immutable" BEFORE UPDATE ON "OcrPage"
WHEN NEW."documentId" <> OLD."documentId" OR NEW."pageNumber" <> OLD."pageNumber" OR NEW."rasterObjectKey" <> OLD."rasterObjectKey" OR NEW."rasterSha256" <> OLD."rasterSha256" OR NEW."sourceDigest" <> OLD."sourceDigest" OR NEW."sourceWidth" <> OLD."sourceWidth" OR NEW."sourceHeight" <> OLD."sourceHeight" OR NEW."sourceRotation" <> OLD."sourceRotation" OR NEW."pixelCount" <> OLD."pixelCount"
BEGIN SELECT RAISE(ABORT, 'OCR_PAGE_SOURCE_EVIDENCE_IMMUTABLE'); END;

CREATE TRIGGER "OcrCandidate_source_evidence_immutable" BEFORE UPDATE ON "OcrFieldCandidate"
WHEN NEW."documentId" <> OLD."documentId" OR COALESCE(NEW."pageId",'') <> COALESCE(OLD."pageId",'') OR NEW."fieldKey" <> OLD."fieldKey" OR NEW."candidateText" <> OLD."candidateText" OR NEW."candidateSha256" <> OLD."candidateSha256" OR COALESCE(NEW."sourceRegionJson",'') <> COALESCE(OLD."sourceRegionJson",'') OR COALESCE(NEW."recognitionScore",-1.0) <> COALESCE(OLD."recognitionScore",-1.0) OR NEW."scriptHint" <> OLD."scriptHint" OR NEW."validationState" <> OLD."validationState" OR NEW."reviewState" <> OLD."reviewState" OR NEW."critical" <> OLD."critical" OR NEW."retryPreprocessing" <> OLD."retryPreprocessing" OR NEW."createdAt" <> OLD."createdAt"
BEGIN SELECT RAISE(ABORT, 'OCR_CANDIDATE_SOURCE_EVIDENCE_IMMUTABLE'); END;

CREATE TRIGGER "OcrWorkflowEvent_no_update" BEFORE UPDATE ON "OcrWorkflowEvent"
BEGIN SELECT RAISE(ABORT, 'OCR_WORKFLOW_EVENT_IMMUTABLE'); END;
CREATE TRIGGER "OcrWorkflowEvent_no_delete" BEFORE DELETE ON "OcrWorkflowEvent"
BEGIN SELECT RAISE(ABORT, 'OCR_WORKFLOW_EVENT_IMMUTABLE'); END;
