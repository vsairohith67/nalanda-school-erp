...
export function ParentMeetingParentPortal({ initialData }: { initialData: any }) {
  ...
  async function requestMeeting(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      await refresh(); formElement.reset(); setMessage({ text: "Your meeting request was recorded for the selected linked child." });
    } catch (error) { setMessage({ error: true, text: error instanceof Error ? error.message : "The meeting request failed safely." }); }
    finally { setBusy(false); }
  }

  async function cancel(event: FormEvent<HTMLFormElement>, meeting: any) {
    ...
    try {
      ...
      await refresh(); setCancelKey(null); setMessage({ text: "The request was cancelled with its history preserved." });
