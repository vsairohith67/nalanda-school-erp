...
export function NamedUserDetail({ detail, decisions, roles, profiles, permissions, canManage }: {
  detail: Detail;
  decisions: Decision[];
  roles: Option[];
  profiles: Profile[];
  ...
  return (
    <div className="iam-workspace">
      ...
      {canManage ? <section className="card iam-action-grid">
        <form onSubmit={(event) => submitStructured(event, "ASSIGN_ROLE")}><h3>Add base role context</h3><label>Role<select name="role" required defaultValue=""><option value="">Choose role</option>{roles.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></label><label>Valid until<input name="validUntil" type="datetime-local" /></label><CriticalFields /><button disabled={busy}>Assign role</button></form>
        <form onSubmit={(event) => submitStructured(event, "ASSIGN_PROFILE")}><h3>Assign permission profile</h3><label>Profile<select name="profileHandle" required defaultValue=""><option value="">Choose profile</option>{profiles.map((profile) => <option key={profile.handle} value={profile.handle}>{profile.label}</option>)}</select></label><label>Valid until<input name="validUntil" type="datetime-local" /></label><CriticalFields /><button disabled={busy}>Assign profile</button></form>
        <form onSubmit={(event) => submitStructured(event, "SET_OVERRIDE")}><h3>Individual grant or denial</h3><label>Permission<select name="permission" required defaultValue=""><option value="">Choose permission</option>{permissions.map((permission) => <option key={permission.value} value={permission.value}>{permission.label}</option>)}</select></label><label>Effect<select name="effect"><option value="DENY">Explicit denial</option><option value="ALLOW">Narrow additional grant</option></select></label><label>Valid until<input name="validUntil" type="datetime-local" /></label><CriticalFields /><button disabled={busy}>Set override</button></form>
        <form onSubmit={(event) => submitStructured(event, "UPDATE_IDENTITY")}><h3>Display identity</h3><label>Name<input name="name" defaultValue={detail.name} required maxLength={100} /></label><label>Username<input name="username" defaultValue={detail.username} required maxLength={64} autoComplete="off" /></label><label>Email<input name="email" type="email" defaultValue={detail.email ?? ""} maxLength={254} autoComplete="off" /></label><label>Human designation<input name="designation" defaultValue={detail.designation ?? ""} maxLength={100} /></label><CriticalFields /><button disabled={busy}>Update identity</button></form>
      </section> : null}
      ...
      <section className="card"><h2>Assigned permission profiles</h2><div className="table-wrap"><table><thead><tr><th>Profile</th><th>Status</th><th>Validity</th><th>Action</th></tr></thead><tbody>{detail.profiles.map((profile) => <tr key={profile.handle}><td>{profile.name}</td><td>{label(profile.status)} · profile {label(profile.profileStatus)}</td><td>{profile.validUntil ? new Date(profile.validUntil).toLocaleString("en-IN") : "Open ended"}</td><td>{canManage && profile.status === "ACTIVE" ? <button type="button" className="secondary" onClick={() => setPending({ action: "END_PROFILE", title: `End ${profile.name} assignment?`, assignmentHandle: profile.handle })}>End assignment</button> : "Historical"}</td></tr>)}{!detail.profiles.length ? <tr><td colSpan={4}>No profile assignments.</td></tr> : null}</tbody></table></div></section>
      <section className="card"><h2>Individual grants and explicit denials</h2><div className="table-wrap"><table><thead><tr><th>Permission</th><th>Source</th><th>Status / validity</th><th>Action</th></tr></thead><tbody>{detail.overrides.map((override) => <tr key={override.handle}><td>{permissionLabels.get(override.permission) ?? "Governed permission"}</td><td>{override.effect === "DENY" ? "Individual denial" : "Individual grant"}</td><td>{label(override.status)}{override.validUntil ? ` · until ${new Date(override.validUntil).toLocaleString("en-IN")}` : ""}</td><td>{canManage && override.status === "ACTIVE" ? <button type="button" className="secondary" onClick={() => setPending({ action: "REVOKE_OVERRIDE", title: "Revoke this individual override?", overrideHandle: override.handle })}>Revoke override</button> : "Historical"}</td></tr>)}{!detail.overrides.length ? <tr><td colSpan={4}>No individual overrides.</td></tr> : null}</tbody></table></div></section>
