...
export function ExaminationConfigurationWorkspace({
  ...
  const draftComponents = draftSchemes.flatMap((scheme) => scheme.components.map((component) => ({
    ...
    schemeSubjectPaperId: scheme.subjectPaperId
  })));
  ...
  async function mutate(path: string, payload: Record<string, unknown>, label: string, method = "POST") {
    ...
    try {
      const response = await fetch(path, {
        ...
        body: JSON.stringify({
          ...payload,
          expectedExaminationVersion: examination.version,
      ...
      {canManage ? (
        <>
          <section className="card card-pad">
            ...
            <p className="muted">Choose raw sum or weighted and normalised explicitly. No maxima or historical weights are prefilled.</p>
            <div className="form-grid">
              <label>
                Calculation mode
                <select value={calculationMode} onChange={(event) => setCalculationMode(event.target.value)} required>
                  <option value="">Select mode</option>
                  <option value="RAW_SUM">{calculationModeLabel("RAW_SUM")}</option>
                  <option value="WEIGHTED_NORMALIZED">{calculationModeLabel("WEIGHTED_NORMALIZED")}</option>
                </select>
              </label>
            ...
            {scope.schemeVersions.length ? (
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>{scope.schemeVersions.map((scheme) => <tr key={scheme.id}><td>v{scheme.versionNumber}</td><td>{calculationModeLabel(scheme.calculationMode)}</td><td>{scheme.components.map((component) => `${component.name} / ${component.maximumMarks}${component.contributionWeight ? ` / ${component.contributionWeight}%` : ""}`).join(", ")}</td><td>{configurationStatusLabel(scheme.status)}</td><td>{roundingPolicyLabel(scheme.roundingPolicyVersion)}</td><td><button type="button" className="button secondary" disabled={busy !== ""} onClick={() => mutate(`/api/exam-configurations/${examination.id}/schemes`, { classScopeId: scope.id, cloneSourceId: scheme.id }, `Scheme v${scheme.versionNumber} cloned`)}>Clone</button></td></tr>)}</tbody>
                </table>
              </div>
            ) : null}
          </section>
      ...
      <section className="card card-pad">
        ...
        <div className="grid two">
          <div>
            ...
            <ul>{scope.subjectGroups.map((group) => <li key={group.id}>{group.groupName} · {calculationModeLabel(group.calculationMode)} · {group.members.map((member) => member.subjectPaper.paperName).join(", ")}</li>)}</ul>
          </div>
          <div>
            <h3>Grade scheme</h3>
            <ul>{scope.gradeScaleVersions.map((scale) => <li key={scale.id}>v{scale.versionNumber} {scale.name} · {scaleFamilyLabel(scale.scaleFamily)} · {configurationStatusLabel(scale.status)} · {scale.bands.map((band) => `${band.gradeCode} ${band.minimumPercentage}-${band.maximumPercentage}`).join(", ")}</li>)}</ul>
            <h3>Co-scholastic scheme</h3>
            <ul>{scope.coScholasticVersions.map((scheme) => <li key={scheme.id}>v{scheme.versionNumber} {scheme.name} · {coScholasticFamilyLabel(scheme.schemeFamily)} · {configurationStatusLabel(scheme.status)} · {scheme.items.map((item) => item.label).join(", ")}</li>)}</ul>
            <h3>Template family</h3>
            <ul>{scope.templateBindings.map((binding) => <li key={binding.id}>v{binding.versionNumber} {templateFamilyLabel(binding.templateFamily)} · {evidenceStatusLabel(binding.evidenceStatus)} · {configurationStatusLabel(binding.status)}</li>)}</ul>
          </div>
        </div>
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>{scope.teacherAssignments.map((assignment) => <tr key={assignment.id}><td>{assignment.subjectPaper.paperName}</td><td>{assignment.component.name}</td><td>{assignment.staffMember.displayName ?? assignment.staffMember.fullName}</td><td>{assignmentRoleLabel(assignment.assignmentRole)}</td><td>{configurationStatusLabel(assignment.status)}</td></tr>)}{!scope.teacherAssignments.length ? <tr><td colSpan={5}>No exact Teacher assignments recorded.</td></tr> : null}</tbody>
          </table>
        </div>
      </section>
      ...
      {canActivate && activationScheme ? (
        <section className="card card-pad">
          ...
          <form onSubmit={(event) => {
            ...
            setConfirmation({
              title: "Activate and freeze this scheme?",
              description: "The selected version and its bound grade, co-scholastic, template, and ownership configuration will become frozen history. Corrections require a new version.",
              confirmLabel: "Activate and Freeze",
              path: `/api/exam-configurations/${examination.id}/workflow`,
              payload: {
              ...
              successLabel: "Scheme activated and frozen"
            });
          }} className="form-grid">
            <label>
              Draft scheme
              <select name="schemeVersionId" required>
                {draftSchemes.map((scheme) => <option value={scheme.id} key={scheme.id}>Version {scheme.versionNumber} · {calculationModeLabel(scheme.calculationMode)}</option>)}
              </select>
            </label>
        ...
        <section className="card card-pad">
          ...
          <form className="form-grid" onSubmit={(event) => {
            ...
            setConfirmation({
              ...
              description: "The examination and its configuration history will be retained, but the archived examination will no longer accept configuration changes.",
              confirmLabel: "Archive Without Deleting",
              danger: true,
              path: `/api/exam-configurations/${examination.id}/workflow`,
              payload: {
              ...
              successLabel: "Examination archived"
            });
          }}>
      ...
      <section className="card card-pad">
        ...
        <div className="table-wrap">
          <table>
            ...
            <tbody>{examination.schemeAudits.map((audit) => <tr key={audit.id}><td>{new Date(audit.eventDate).toLocaleString("en-IN")}</td><td>{configurationStatusLabel(audit.eventType)}</td><td>{configurationStatusLabel(audit.targetType)}</td><td>{audit.previousStatus ? configurationStatusLabel(audit.previousStatus) : "—"} → {audit.newStatus ? configurationStatusLabel(audit.newStatus) : "—"}</td><td>{configurationStatusLabel(audit.actorRole)}</td><td>{audit.reason ?? "—"}</td></tr>)}</tbody>
          </table>
        </div>
      </section>
      {confirmation ? (
        <div className="confirmation-overlay" role="presentation">
          ...
          >
            <h3 id="exam-confirmation-title">{confirmation.title}</h3>
            <p id="exam-confirmation-description">{confirmation.description}</p>
            <div className="page-actions">
              <button type="button" className="button secondary" autoFocus disabled={busy !== ""} onClick={() => setConfirmation(null)}>Go Back</button>
              <button
                type="button"
                className={confirmation.danger ? "button danger" : "button"}
                disabled={busy !== ""}
                onClick={async () => {
                  const action = confirmation;
                  setConfirmation(null);
                  await mutate(action.path, action.payload, action.successLabel, action.method);
                }}
              >
...
function AssignmentForm({ examinationId, scope, teachers, draftComponents, busy, mutate }: { examinationId: string; scope: Scope; teachers: Option[]; draftComponents: Array<ComponentRow & { schemeId: string; schemeVersion: number; schemeSubjectPaperId: string | null }>; busy: string; mutate: Mutate }) {
  const [subjectPaperId, setSubjectPaperId] = useState("");
  const applicableComponents = draftComponents.filter((component) => (
    !component.schemeSubjectPaperId || component.schemeSubjectPaperId === subjectPaperId
  ));
  return <section className="card card-pad"><h2>7. Exact Teacher assignment</h2><p className="muted">The server requires an active staff record, active timetable Teacher, and exact timetable assignment for this year, class, section, and subject.</p><form className="form-grid" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); void mutate(`/api/exam-configurations/${examinationId}/assignments`, { classScopeId: scope.id, subjectPaperId: form.get("subjectPaperId"), componentId: form.get("componentId"), staffMemberId: form.get("staffMemberId"), assignmentRole: form.get("assignmentRole"), assignmentReason: form.get("assignmentReason") }, "Exact Teacher assignment created"); }}><label>Subject paper<select name="subjectPaperId" required value={subjectPaperId} onChange={(event) => setSubjectPaperId(event.target.value)}><option value="" disabled>Select paper</option>{scope.subjectPapers.map((paper) => <option value={paper.id} key={paper.id}>{paper.paperName}</option>)}</select></label><label>Applicable draft scheme component<select name="componentId" required defaultValue="" key={subjectPaperId}><option value="" disabled>Select component</option>{applicableComponents.map((component) => <option value={component.id} key={component.id}>v{component.schemeVersion} · {component.name}</option>)}</select></label><label>Teacher staff link<select name="staffMemberId" required defaultValue=""><option value="" disabled>Select Teacher</option>{teachers.map((option) => <option value={option.id} key={option.id}>{option.label}</option>)}</select></label><label>Ownership role<select name="assignmentRole" required defaultValue=""><option value="" disabled>Select role</option>{["PRIMARY_SUBMITTER", "CONTRIBUTOR"].map((value) => <option key={value} value={value}>{assignmentRoleLabel(value)}</option>)}</select></label><label className="full">Assignment reason<input name="assignmentReason" required maxLength={1_000} /></label><div className="full page-actions"><button disabled={busy !== "" || !subjectPaperId || !applicableComponents.length || !teachers.length} type="submit">Assign Exact Ownership</button></div></form></section>;
}
