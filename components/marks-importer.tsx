...
export function MarksImporter({ governedAssignments }: { governedAssignments?: any[] }) {
  ...
  return <section className="card card-pad" style={{ minWidth: 0, overflowWrap: "anywhere" }}>
    <h3>{governed ? "Governed component draft CSV import" : "Legacy assessment CSV import"}</h3><p>{governed ? "Frozen scheme and eligible assignment. CSV / safe XLSX v1, at most 200 rows. Import saves a draft only." : "Legacy nine-column CSV, at most 2000 rows. Select the exact authorised year, exam, class, section, subject and component."}</p>
