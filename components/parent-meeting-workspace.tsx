...
export function ParentMeetingWorkspace({ initialData }: { initialData: Workspace }) {
  ...
  async function correctNote(note: any) {
    if (!selected) return;
    const body = window.prompt("Corrected note text (the original remains in history)", note.body);
    if (!body || body === note.body) return;
    const correctionReason = window.prompt("Reason for this preserved correction");
    if (!correctionReason) return;
    await send(`/api/parent-meetings/${selected.publicKey}/notes`, { kind: note.kind, body, correctsNoteKey: note.publicKey, correctionReason }, "The correction was appended without erasing the original note.");
  }
  ...
  async function workflow(action: string) {
    ...
    if (action === "CANCEL") {
      const internalReason = window.prompt("Internal cancellation reason (not shown to Parent)");
      if (!internalReason) return;
      body.internalReason = internalReason;
      body.parentSummary = window.prompt("Optional Parent-visible cancellation summary") || "";
    }
