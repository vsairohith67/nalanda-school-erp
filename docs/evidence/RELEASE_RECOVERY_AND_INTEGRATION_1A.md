...
## Recovery continuation - 19 September 2026

Origin main remains 104aacc7bd314cae82e60bb02b5c8a965c7ffedd; integration PR28 remains open at e84d8c3 before this checkpoint. The primary local checkout now points to an older main commit e8ed293; it was inspected read-only and is not our integration/release authority. PR27 advanced to 1abdb0e46173ab5cd4fab6eda0b1ec0f79c8378c via 17d738a, 2d40a7c and 1abdb0e. Read-only independent review found bounded dependency/CI/privacy/governance fixes; controlled admission follows this checkpoint, preserving the integrated pins and union contracts.

The September 12 full local regression produced 2,391 passed, 56 failed and three governed skips out of 2,450 tests. Failures were retained, not marked passed. Most failing domain fixtures fabricated obsolete versions or changed rows without matching counts. Updated tests retain domain/privacy assertions using a complete current fixture, explicitly reject unsupported historical labels, and maintain direct malformed-envelope tests. The count helper only clones an already-current fixture and updates existing array counts; it never changes source identity, fields, collections or version. Genuine v45/v46/v47 compatibility continues to use original archived exporters and actual schema migrations. Targeted rerun: 388/390 passed; two residual fixture errors repaired, restore-unit suite 18/18 passed.

Hosted e84d8c3 results are terminal: Windows unsigned NSIS, Android emulator and iOS simulator jobs PASS; schema/migration and provider parity jobs PASS. Full application regression jobs FAILED; these are being repaired. PostgreSQL genuine v45/v46/v47 restore cases passed; the v48 interruption case failed because its synthetic schema identifier exceeded 63 characters. The bounded naming repair preserves the unique run suffix. No Browser/HTTP or image-dependent job is represented as passed.

Independent review identified omitted certificate charge history despite intact issued snapshots. Validation now requires each Graduation issued version's referenced charge, immutable rate/status/receipt linkage, approver and approval time. Reissue legitimately reuses the original charge. Adversarial tests delete charge rows while adjusting counts and mutate approval proof; interrupted restore now faults on the second artifact, asserts the fault occurred and checks financial/document/event rollback before retry.

Fresh all/prod/dev exact-lock audits report zero findings separately. Official September 19 checks retain Next15.5.25, Sharp0.35.4 and Vitest4.1.11; Vitest4.1 still receives important/security fixes. The public-source scan passes 270 changed files with zero detected private binaries/secrets/contacts, and Git safety passes. The two amended native workspace manifests are now explicitly scanned rather than omitted.

Runtime recheck: EXTERNAL_RUNTIME_BLOCKED, now UPSTREAM_FIX_AVAILABLE_VENDOR_AND_NODE24_RELEASE_PENDING. Upstream zlib commit df84af25dc1942490e1d1c899a07619152a46148 dated September17 fixes CVE-2026-85091 (https://github.com/madler/zlib/commit/df84af25dc1942490e1d1c899a07619152a46148). Debian stable packages remain unfixed (https://security-tracker.debian.org/tracker/CVE-2026-85091); its glibc5435 stable hold remains (https://security-tracker.debian.org/tracker/CVE-2026-5435). Node24.21.0 still bundles the unchanged affected gzwrite.c; a supported corrected release/image is not established. Ubuntu's September16 no-upstream-fix note predates the new commit and is not used as current upstream evidence. No rebuild, waiver or full-stack bypass.
...
PR27 drift admission, September19: after checkpoint c41d116, normal merge admits committed 05e6b5c6021d57f6081f2df692a8d29dd9e784f9 (387f945 and05e6b5c). The active feature task retains ownership of its worktree; no dirty files are copied. The five-file delta tightens native capture readiness, preserves Android FLAG_SECURE, targets the observed PIN input, and rejects blank iOS captures. Independent review found this bounded and compatible. Pixel variation is only a negative blank-capture check; it does not establish functional workflow acceptance or whole-image confidentiality. Earlier green native jobs cannot close the newly exposed capture-evidence gap. Historical 1abdb0e source inventory remains preserved separately; the integration source-head ledger now records05e6b5c. Any later feature drift stays excluded pending separate review. No runtime gate was removed.

Final QA repair: all local TypeScript partitions and production compile/generate build PASS after completing the fixture's explicitly empty optional collections. Both full v45-v48 financial/recovery provider jobs PASS at 84483de, including new ownership/collision/interruption cases. Two hosted public scanners refused the newly reviewed vitest.config.ts path; exact-path admission retains their content scanning. Four focused tests PASS, including private-key injection into that config and unknown sibling rejection; both public scanners PASS over277 changed files. Independent review confirmed this is scope admission, not a secret-scan suppression. The final candidate needs its own terminal hosted checks; earlier failures remain preserved.
