...
### Final evidence inspection and native blank-capture gate — 19 September 2026

Retained head 1abdb0e46173ab5cd4fab6eda0b1ec0f79c8378c completed all nine workflows / 25 GitHub checks successfully. Exact-head HTTP run34704719585 passed four production-OFF and 24 synthetic-ON assertions plus complete teardown. Master run34704721714 passed all 250 test files / 2324 tests with no skips, all typecheck partitions, build and v45 backup/restore. PostgreSQL/cross-provider and all six portable jobs passed. The earlier local fixture/qpdf omissions remain historical; the hosted full run executed them. Fresh dependency audit passes the High/Critical threshold with two Moderate test-tool advisories. The separate shared-runtime advisory conflict is not adjudicated by these results.

Visual inspection of native artifacts in run34704721770 found Android portrait/offline blank and an empty accessibility WebView, and iPhone dark blank. iPhone light and iPad dark visibly show the bundled PIN screen. Blank output is not expected for NO_REMOTE_SERVER_CONFIGURED. The original Android fallback accepted WebView presence without content; iOS captured after fixed sleeps. Consequently that green workflow proves build/install/launch only, not complete rendered-content acceptance. Independent review confirmed this evidence gap; its root cause (rendering race versus runtime failure) is unproven.

The narrow QA correction removes the empty-WebView fallback, requires Android PIN-screen accessibility text, and retries fresh screenshots within bounded windows. An existing-Sharp helper rejects blank central image content while excluding OS bars; synthetic blank/OS-bar, nonblank and malformed-image tests pass. This is only a negative blank-capture check, not OCR, semantic-control verification or import acceptance. iOS light mode is selected explicitly before its capture. Synthetic launch errors are retained even after early Android failure. No native application engine, remote profile, IPC, encrypted finance workflow or production configuration changes. Final-head native execution and manual artifact review remain mandatory; a persistent blank fails rather than becoming a false pass.

The protected database was rehashed on 19 September and exactly matches the supplied SHA/size/mtime/no-sidecar contract. Main remains104aacc7; parked PR19/24/25 and finance PR26 retain their exact heads. PR28 is a separate recovery workstream and was not integrated. Final post-correction SHA and results are recorded on PR27 and in the tracker handoff; installed Windows, native import picker/download and physical acceptance remain open.


### Native privacy correction and source-contract reconciliation

Run35449773451 on387f945 produced a fresh Android accessibility hierarchy containing the PIN heading, input and button. Its screenshot remained black. Source inspection then established that scripts/harden-cross-platform-generated-project.mjs deliberately applies Android FLAG_SECURE: black screenshots are expected privacy masking, not evidence of a blank app. This corrects the earlier image-based inference above. The previous empty accessibility tree was a real premature-readiness gap; requiring visible PIN text addresses it. The first negative screenshot guard was incorrectly applied to Android and its failure is retained, not relabelled a product defect.

The corrected gate preserves FLAG_SECURE and separately checks fresh PIN accessibility content for each Android state plus masked portrait pixels before keyboard interaction. Later captures may legitimately include the OS keyboard, so they are not asserted wholly masked. iOS retains nonblank capture retries. PIN focus targets the observed input bounds rather than a guessed coordinate. No screenshot protection is disabled for QA. Neither accessibility nor pixel evidence certifies authenticated native import/picker/download or physical UX.

Several current-head workflows also exposed one existing source assertion that explicitly required the old empty-WebView fallback. The test now forbids that fallback and requires the stronger PIN/protected-capture and iOS checks. Focused product/native/runtime tests32 and master reconciliation13 pass, with affected typecheck passing. Finance/parked PR path inventories show no collision on this product-experience test; no foreign code is copied. All next-head workflows remain mandatory. Authenticated synthetic run35449771252 on387f945 passed all28 OFF/ON checks, readback/downloads and teardown; application source is unchanged by these QA corrections.

The iOS job on387f945 subsequently passed all four iPhone/iPad light/dark nonblank captures; manual inspection confirms the previously blank iPhone dark capture now shows the PIN screen, as does iPad light. Windows unsigned NSIS compilation passed. Independent review cleared the privacy-preserving Android correction; its next exact-head execution remains required.
