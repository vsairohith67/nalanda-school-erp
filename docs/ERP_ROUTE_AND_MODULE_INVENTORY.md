...
| `/payments/[id]/edit` | Payments | Review/edit/cancel/restore payment workflow | `EDIT_PAYMENTS`, `CANCEL_PAYMENTS`, `RESTORE_PAYMENTS` | Stable | Medium | Preserve audit language and reversible controls. |
...
| `/pending-dues` | Dues/Reports | Due position, filters, reminders, CSV export | `VIEW_PENDING_DUES`; reminders via `COMMUNICATE_PARENT` | Stable, needs UI polish | High | Large table and action-heavy rows; mobile risk is high. |
...
| `/ledger` | Student Ledger | Student ledger search | `VIEW_LEDGER` | Stable, needs UI polish | Medium | Search and ledger table need mobile review. |
| `/ledger/print` | Student Ledger | Printable ledger | `PRINT_LEDGER` | Stable | Low | Print route. |
| `/receipt-audit` | Receipt/Payment Audit | Missing/duplicate/split/cancel/reference checks | `VIEW_RECEIPT_AUDIT`; notes via `MANAGE_RECEIPTS` | Stable, needs UI polish | Medium | Important warnings could become dashboard widgets later. |
