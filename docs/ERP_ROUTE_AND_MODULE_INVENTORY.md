...
| `/payments/[id]/edit` | Payments | Audited correction plus whole-receipt cancel/restore | `EDIT_PAYMENTS`; final cancellation requires `CANCEL_PAYMENTS` and Director/Super Admin | Stable | Medium | Receipt/admission are locked; reason, version, accessible dialog, transaction, and append-only audit are mandatory. |
...
| `/receipt-audit` | Receipt/Payment Audit | Missing/duplicate/split/note-drift/cancel/reference checks | `VIEW_RECEIPT_AUDIT`; final cancellation requires `CANCEL_PAYMENTS` and Director/Super Admin | Stable, needs UI polish | Medium | Payment components are authoritative; mixed component state or ReceiptNote disagreement fails closed. |
