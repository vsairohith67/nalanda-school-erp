...
| Workflow correctness | Operational database has no Prisma baseline metadata | Medium | DEVOPS-1B repairs fresh installs with a generated SQLite baseline and proves copied-database onboarding twice; the real operational database is intentionally untouched | Independent QA, backup/hash/schema proof and a separately approved maintenance window are required before operational `migrate resolve`; cloud/Postgres remains a separate migration plan. |
