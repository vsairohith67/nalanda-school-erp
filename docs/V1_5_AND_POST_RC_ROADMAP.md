...
`SUPER-ADMIN-COMMAND-1A — Super Admin Command Center Foundation`

This phase can start before hosting using synthetic data and existing read-only APIs. It should compose the cleared dashboard, OBS-1A monitoring, role controls, academic operations, and provider-status foundations without duplicating them. Hosting and private staging remain a parallel operational track, not a prerequisite for beginning this product phase.
...
| 3 | V1.5 product | `SUPER-ADMIN-COMMAND-1A — Super Admin Command Center Foundation` | Build one permission-scoped, read-only command surface over existing dashboards and operational signals. | Cleared dashboard, OBS-1A, IAM, academic calendar, and existing APIs | 5–8 working days | **Yes — recommended immediate phase** | Role isolation, source attribution, empty/error/loading states, responsive layout, audit-safe navigation, and no write side effects | Read-only Command Center foundation ready for independent focused QA |
| 4 | V1.5 product | `SUPER-ADMIN-WORK-1A — Diary, Tasks, Reminders and Directory` | Turn the Notion diary pattern, tasks, reminders, Life OS concepts, and publisher/vendor/contact directory into one governed Command Center product model. | Phase 3 scope accepted; product ownership and retention rules decided | 8–12 working days | Yes | Permission tests, lifecycle tests, reminder idempotency, PII minimisation, contact deduplication, export/backup coverage | Governed work and directory foundation accepted with synthetic data |
