...
| RC-TD-07 | R7 stacked metric labels, dense marks allocation and 12 mm signature clearance did not meet final table-priority direction | Resolved in R8 with measured single-line cells, dense academic-table priority, combined VI-X trait/grade, adaptive chart footprint and 15 mm signing clearance; digital and physical acceptance passed |
| RC-TD-08 | Future visual or calculation edits could drift an accepted issued template | Governed by `REPORT_CARD_TEMPLATE_VERSIONED_MAINTENANCE_POLICY.md`: new inactive draft version, proportional QA, explicit activation, immutable issued snapshots and retained known-good rollback |
