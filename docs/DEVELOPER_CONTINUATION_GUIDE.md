...
This local database is not migration-baselined: `prisma migrate deploy` returns P3005 on the existing non-empty SQLite database. Prompt 15B retained the SQL migration for future/baselined environments and used the repo's established additive `pnpm.cmd db:push` path locally. Do not mark old migrations as applied casually; production/cloud baselining remains separate technical debt.
...
- `prisma/migrations/` — existing schema migrations.
...
- `prisma db push` is convenient for a fresh local setup but is not a substitute for reviewed production migrations.
- Existing migrations were added across phases and may not represent every later schema adjustment if `db push` was used. Compare the current schema with a fresh test database before any cloud/database migration.
