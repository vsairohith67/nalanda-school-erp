...
## DEVOPS-1D synthetic staging deployment

Work is on `devops/synthetic-staging-deployment`. AWS is excluded without resource creation. Vultr Mumbai is the current technical recommendation and Linode Mumbai is fallback, but all account/payment/billable actions are deferred until the costs and concepts are explained and separately approved. Provider-neutral templates now use `/opt/nalanda` releases, persistent `/var/lib/nalanda` data/uploads/backups, `/var/cache/nalanda/next`, `/var/log/nalanda`, root-owned `/etc/nalanda/staging.env`, hardened systemd and Caddy 2.10+ exact body limits. The synthetic seed/check design covers distinct QA Director, Principal, Teacher and Parent accounts plus a linked Student.

Actual staging deployment did not occur. No provider account, payment method, VPS, paid resource, DNS record, deploy key, server environment, staging database, off-host backup, external monitoring/logging, operational upload or live provider exists. Only no-cost local synthetic preparation and rehearsal were completed. DEVOPS-1D is incomplete, DEVOPS-1D-QA has not begun, and physical PWA certification must not begin. Any future paid deployment requires new explicit approval.
