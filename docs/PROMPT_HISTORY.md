...
The exact copied-data profile passed with 800 Students, 1,200 Guardians, 80 Staff, 45 Teachers, 1,600 enrollments, 80 year/cohort combinations, sibling groups of two/three/four, 4,480 lineage outcomes, idempotent replay and exact rollback in 8,032 ms. Full split-project typecheck, 215 passing test files / 1,913 passing tests, production build with 283 generated pages, 333 page routes / 547 APIs, migration/restore and Release Operations rehearsal all passed. Three qpdf runtime tests remain deliberately skipped until the required binary/hash is operationally configured; the workflow fails closed without it.

Representative in-app Browser QA covered Super Admin, Parent, Teacher/Parent and Viewer at desktop and exact 390×844, light/dark, linked-child/read-only state, release boundaries, privileged-route denial, keyboard focus and overflow. It found and closed one production presentation defect: Teacher classwork without an active timetable scope now renders a labelled fail-closed empty state rather than a generic Server Component error, without relaxing API authorization. Final Browser console warnings/errors were zero, copied fixtures were removed twice, and the protected zero-business/four-account operational fingerprint remained exact. The feature branch is prepared for independent `V1-FINAL-1A-QA`; no merge, tag, deployment, live provider, real data or real-user activation is authorized by this phase.
...
Independent RELEASE-OPS-1A-QA closed generic dirty-form update safety, deterministic source-time packaging, ignored restart-state defaults and strict gate/rollback/maintenance sequencing. The final gate passed 15 focused tests, full typecheck, 1,792 tests across 203 passing files, a 333-page/547-API production framework build, all 18 active migrations on fresh/copied databases, restore twice, reproducible package verification, lock/low-space/audit failure injection, exact operational baseline preservation, Browser role/theme/exact-390x844 evidence, backup v41 and Git safety. Status: `RELEASE_OPERATIONS_CLEARED`; retained branch and annotated `release-operations-v41-2026-08-10` identify the cleared tree. No cloud resource, hosting purchase, DNS change, provider activation, operational-data upload, public exposure, real-user onboarding or native app release is authorised or performed. Next governed phase: `REPORT-PRINT-ACCEPT-1A`.
...
## SAFE-EXIT-1A — Student Early Departure, Gate Pass and Guardian Notification (2026-08-09)

Implemented an additive, safety-governed early-departure workflow with linked-Parent or verified Staff-recorded consent, school approval, governed handover/self-departure, opaque signed single-use gate pass, separate campus-presence history, exact Parent/Teacher/Staff/Gate object scope, restricted emergency override and unauthorised-exit incident, authoritative in-app notification plus local push/WhatsApp outbox, leadership fallback tasks and backup version 38. `SAFEEXIT1` copied-database QA deployed twice and restored twice without touching the zero-business operational baseline. The serialized implementation gate passed 328 page routes, 527 API routes, lifecycle dry run, full typecheck, 1,748 tests, production build, backup and Git safety. No real Student/Guardian/departure/incident, deployment or live provider was used. Status: `STUDENT_SAFE_EXIT_READY_FOR_QA`; independent `SAFE-EXIT-1A-QA` continues in the same thread before any merge/tag. Next governed phase after clearance: `OBS-1A`.

Independent `SAFE-EXIT-1A-QA` then reviewed every changed safety path and added fail-closed origin/body handling, current Guardian/standing-consent checks, approval snapshot staleness, incident/pass separation, same-day presence and retry/fallback recovery. `SAFEEXIT1QA` and fresh Browser fixtures proved normal and emergency flows, unauthorised incidents, exact role/context privacy, signed opaque single-use/manual passes, concurrent checkout, exactly-once local notification sinks, return/presence history and attendance separation. Production Browser QA covered all required roles, 1366 x 768 and 390 x 844, light/dark, keyboard/44 px/overflow, print, manual Gate checkout and pass reuse denial with zero final console/stderr errors. The additive migration was rehearsed and deployed twice with byte-identical rollback proof; zero business/Safe Exit rows and all four protected accounts remain unchanged. The retained feature branch, `main` and `student-safe-exit-v38-2026-08-09` close with `STUDENT_SAFE_EXIT_CLEARED`. No real departure, live WhatsApp provider, deployment or real-data onboarding was used. Next governed phase: `OBS-1A`.

Historical terminal-token aliases for this same governed release are `STUDENT_EARLY_EXIT_CLEARED`, `STUDENT_EARLY_DEPARTURE_CLEARED` and `STUDENT_SAFE_EXIT_CLEARED`; they do not represent separate implementations or releases.

## OBS-1A — Technical Operations, Safe Logging and Super Admin Observability (2026-08-10)

Implementation adds a provider-neutral, no-store technical control plane over existing authentication, IAM, notifications, backup/recovery, migration, provider, document, storage, PWA and deployment primitives. New additive records cover stable check definitions, bounded runs/metrics, deduplicated alerts, incident/maintenance timelines, immutable release manifests, advisory client policy and generic background-job status. Exact permissions separate Director summary, full evidence and actions. Critical alerts reuse in-app notifications; external telemetry and provider activation remain disabled. Logical backup format advances to v40 for durable operational configuration/history while high-volume snapshots remain retention-bound. Status: implementation checkpoint; independent `OBS-1A-QA` is required before merge/tag or clearance.
...
The feature remains server-side `OFF` at `0%`. Focused copied-database QA passed and left the operational SQLite byte-identical. Independent security/browser/platform/full-regression/exact-head CI passed; PR #21 merged normally at `78a889ec5ddf952d507fff4fb47ac38ef207eced`, and annotated `real-user-access-readiness-v44-2026-09-04` was created. No real user/contact/invitation/MFA, live delivery provider, private staging, deployment, OCR change or physical-device certification is included. The generated `REAL-USER-ACCESS-ACTIVATION-1B` prompt is a do-not-run future gate requiring private infrastructure and per-wave owner approval. Result: `REAL_USER_ACCESS_READINESS_1A_CLEARED`; operational activation remains inactive.
