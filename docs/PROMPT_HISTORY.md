...
## DEVOPS-1D — Supervised synthetic HTTPS staging deployment (in progress, 2026-07-24)

Pre-flight passed on clean, synchronized main commit `44ab30bde1298035b58fcce0a8aacc6ea9c95705`, then work moved to `devops/synthetic-staging-deployment`. AWS account completion was unacceptable and AWS was excluded without creating a resource. Public pricing research identified Vultr Mumbai as the technical replacement recommendation and Linode Mumbai as fallback, but the user paused every provider-account, payment-method and billable action until the concepts and charges are explained later.

Unpaid preparation corrected the provider-neutral layout to `/opt/nalanda`, `/var/lib/nalanda`, `/var/cache/nalanda`, `/var/log/nalanda` and `/etc/nalanda/staging.env`; hardened the systemd template; added exact Caddy 2.10+ body-limit/proxy sanitization; expanded the synthetic seed/check design to distinct Director, Principal, Teacher and Parent accounts plus a linked Student; clarified supervised-only backup/monitoring limits; and created the deployment-report record. No provider account/payment method/VPS, DNS, deploy key, off-host backup, external monitor, staging secret/database, operational upload or live provider was created.

The no-cost local verification passed 274 page routes, 377 API routes, lifecycle/migration/schema/environment checks, typecheck, Git safety, a fresh idempotent synthetic-only rehearsal, 1,477 tests across 162 files and an optimized 211/211-page build. Local backup version 37 `nalanda-fee-control-backup-2026-07-24-01-43.json` was created and remains prohibited from staging. The operational database checkpoint remained unchanged. This is preparation evidence only: actual staging deployment did not occur, and the environment is not hosted, live or externally available.

On 2026-07-26 the preparation was authorised for preservation on the feature branch only, without merge or deployment. Closure verification passed 274 page routes, 377 API routes, 1,477 tests across 162 files, the 25 staging/environment and 8 clean-install checks, typecheck, synthetic rehearsal, and an optimized 211/211-page build. Final local backup version 37 is `nalanda-fee-control-backup-2026-07-26-10-44.json`; it remains ignored and prohibited from staging. DEVOPS-1D is not complete, DEVOPS-1D-QA has not begun, and DEVOPS-1D-QA is not safe to begin. All cloud, DNS, deploy-key, backup and monitoring gates remain pending a new explicit approval.
