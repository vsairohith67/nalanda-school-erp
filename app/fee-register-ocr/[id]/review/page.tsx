...
export default async function FeeRegisterOcrReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [batch, canResolve] = await Promise.all([prisma.feeRegisterOcrBatch.findUnique({ where: { id: (await params).id }, include: ocrBatchInclude }), hasUserPermission(user, "RESOLVE_FEE_REGISTER_OCR_DUPLICATES")]);
  if (!batch) notFound();
