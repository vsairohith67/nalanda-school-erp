...
export default async function SmsEmailReportsPage() {
  const user = await requirePermission("VIEW_SMS_EMAIL_REPORTS"), permissions = await getEffectivePermissions(prisma, user.role), report = await loadSmsEmailReports(prisma);
