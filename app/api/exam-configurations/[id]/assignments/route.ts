...
import { createTeacherExamAssignment } from "@/lib/exam-configurations";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(
      { assignment: await createTeacherExamAssignment(prisma, (await params).id, await request.json(), auth.user) },
      { status: 201 }
    );
  } catch (error) {
