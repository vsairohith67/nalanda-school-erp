...
import { createCoScholasticSchemeVersion } from "@/lib/exam-configurations";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(
      { coScholasticSchemeVersion: await createCoScholasticSchemeVersion(prisma, (await params).id, await request.json(), auth.user) },
      { status: 201 }
    );
  } catch (error) {
