...
import { recordParentMeetingAttendance } from "@/lib/parent-meetings";
...
export async function POST(request: NextRequest, context: { params: Promise<{ meetingKey: string }> }) {
  ...
  try { return parentMeetingJson(await recordParentMeetingAttendance(prisma, auth.actor, (await context.params).meetingKey, await parseParentMeetingJson(request))); }
