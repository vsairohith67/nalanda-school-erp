...
export async function GET() {
  const auth = await requireApiPermission("VIEW_OWN_SUPPORT"); if (auth.response || !auth.user) return auth.response;
  if (auth.user.role === "PARENT" || auth.user.role === "STUDENT") return supportJson({ error: "Use the support portal for the active role context." }, 403);
...
export async function POST(request: NextRequest) {
  const auth = await requireApiPermission("CREATE_OWN_SUPPORT"); if (auth.response || !auth.user) return auth.response;
  if (auth.user.role === "PARENT" || auth.user.role === "STUDENT") return supportJson({ error: "Use the support portal for the active role context." }, 403);
