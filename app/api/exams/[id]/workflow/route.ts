...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { const authority = await resolveMarksWriteAuthority(prisma, auth.user, undefined, permission); if (authority.mode !== "LEADERSHIP") throw new AcademicIntegrityError("Only the Principal or Super Admin may control marks-entry workflow."); const exam = await transitionExam(prisma, (await params).id, action, parseExpectedVersion(body.expectedUpdatedAt, "exam"), auth.user.id, body.reason); return NextResponse.json({ exam: publicExam(exam) }); }
