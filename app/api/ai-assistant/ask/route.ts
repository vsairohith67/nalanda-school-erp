...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!(await hasRolePermission(prisma, auth.user.role, permission))) return NextResponse.json({ error: "This retrieval mode is not authorised for your role." }, { status: 403 });
