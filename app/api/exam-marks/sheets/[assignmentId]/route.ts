...
import { readImportJson, ImportRequestError } from "@/lib/import-request";
import { issueImportReceipt, importDigest, ImportReceiptError } from "@/lib/import-preview-receipt";
...
import { saveAssignedMarkDraft, ExamMarksError } from "@/lib/exam-marks";
...
export async function GET(request: NextRequest, context: { params: Promise<{ assignmentId: string }> }) {
  ...
  } catch(error) { if (error instanceof ImportReceiptError || error instanceof ImportRequestError) return examPrivateJson({ error: error.message }, { status: 400 }); return examMarksApiError(error); }
}
export async function POST(request: NextRequest, context: { params: Promise<{ assignmentId: string }> }) {
  ...
  try {
    ...
    if (body.model !== "GOVERNED_DRAFT") throw new ExamMarksError("Select the governed draft model.");
    ...
    if (body.action !== "confirm") throw new ExamMarksError("Choose preview or confirm.");
