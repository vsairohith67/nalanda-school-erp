...
export async function GET(request: NextRequest) { const auth = await requireApiPermission("VIEW_CASH_BOOK"); if (auth.response) return auth.response; const status = request.nextUrl.searchParams.get("status"); const rows = await prisma.cashBookDay.findMany({ where: status ? { status } : {}, include: cashBookInclude, orderBy: { cashDate: "desc" }, take: 500 }); const sensitive = await hasRolePermission(prisma, auth.user.role, "MANAGE_CASH_BOOK"); return NextResponse.json({ days: await Promise.all(rows.map((row) => cashBookView(prisma, row, sensitive))) }); }
