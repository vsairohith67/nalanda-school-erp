...
export async function GET() { const auth = await requireApiPermission("VIEW_BOOKS_FINANCE"); if (auth.response) return auth.response; const sensitive = await hasUserPermission(auth.user, "MANAGE_BOOK_CASH_SETTLEMENT"); const rows = await prisma.bookCashSettlement.findMany({ include: bookSettlementInclude, orderBy: { settlementDate: "desc" }, take: 500 }); return NextResponse.json({ settlements: await Promise.all(rows.map(async (row) => serializeBookSettlement(row, (await expectedBookCashForDate(prisma, row.settlementDate)).amount, sensitive))) }); }
