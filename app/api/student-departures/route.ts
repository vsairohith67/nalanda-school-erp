...
import { createSafeExitRequest, listSafeExitRequests } from "@/lib/safe-exit";
import { parseSafeExitJson, safeExitApiActor, safeExitApiError, safeExitJson } from "@/lib/safe-exit-api";
export async function GET(){const auth=await safeExitApiActor(["REQUEST_STUDENT_DEPARTURE","VERIFY_GATE_PASS","VIEW_DEPARTURE_AUDIT"]);if(auth.response||!auth.actor)return auth.response;try{return safeExitJson({requests:await listSafeExitRequests(prisma,auth.actor)});}catch(error){return safeExitApiError(error);}}
