...
export async function GET(request: NextRequest, context: { params: Promise<{ assetKey: string }> }) {
  ...
  try {
    ...
    if (variant === "original") {
      ...
      return new NextResponse(bytes, { headers: { ...EVENT_MEDIA_PRIVATE_HEADERS, "Content-Type": asset.originalMediaType, "Content-Disposition": `inline; filename="private-original${asset.originalExtension}"` } });
    }
    ...
    return new NextResponse(bytes, { headers: { ...EVENT_MEDIA_PRIVATE_HEADERS, "Content-Type": derivative.mediaType, "Content-Disposition": "inline; filename=private-thumbnail.jpg" } });
