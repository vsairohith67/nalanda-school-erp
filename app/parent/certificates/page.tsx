...
export default async function ParentCertificatesPage({ searchParams }: { searchParams: Promise<{ student?: string; certificate?: string }> }) {
  ...
  return (
    <div className="page parent-certificates-page">
      ...
      {selected?.certificateType === "GRADUATION" ? <section className="card"><h3>Graduation Certificate — school-issued recognition</h3><p>This institutional recognition does not replace statutory Board or Council documents.</p>{selected.status === "ISSUED" ? <a className="button" href={selected.downloadPath}>Open saved issued PDF</a> : <p>VOID — original history is preserved.</p>}</section> : selected ? <section className="card parent-issued-certificate">
