...
export default async function IdentityCardBatchesPage() {
  const user = await requirePermission("VIEW_ID_CARDS"), permissions = await getCurrentUserEffectivePermissions(), rows = await prisma.identityCardBatch.findMany({ include: { template: { select: { name: true } } }, orderBy: { createdAt: "desc" } });
