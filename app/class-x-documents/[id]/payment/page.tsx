...
export default async function ClassXPaymentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [row, permissions] = await Promise.all([prisma.classXDocumentPackage.findUnique({ where: { id }, include: { student: { select: { studentName: true, admissionNo: true } }, charge: { include: { linkedMiscIncomeReceipt: { select: { id: true, receiptNumber: true, status: true, netAmount: true } } } } } }), getCurrentUserEffectivePermissions()]);
