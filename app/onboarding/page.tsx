...
export default async function OnboardingPage() {
  ...
  return <PageShell className="onboarding-page"><PageHeader title="Bulk Onboarding Centre" description="Private, preview-first Student, Guardian, enrollment and Staff onboarding. Upload never changes business data; execution requires a current plan, separate approval and re-authentication." />{!operationalImportsEnabled ? <p className="notice warning">Operational onboarding execution is not active. Templates, private upload, validation and audit review remain available without applying business records.</p> : null}<OnboardingCentre initialBatches={batches.map((r) => presentBatch(r)) as any} role={user.role} permissions={{ upload, validate, resolve, approve: approve && operationalImportsEnabled, execute: execute && operationalImportsEnabled, audit, rollback }} /></PageShell>;
}
