...
export async function requireAdmissionsAny(permissions: CanonicalPermission[]) { const context = await getCurrentAuthContext(); if (!context) return { response: admissionsJson({ error: "Authentication required" }, 401), user: null, context: null }; if (context.user.mustChangePassword) return { response: admissionsJson({ error: "Password change required" }, 403), user: null, context: null }; for (const permission of permissions) { const decision = await evaluateEffectivePermission(prisma, { userId: context.user.id, sessionId: context.sessionId, roleAssignmentId: context.user.roleAssignmentId, permission }); if (decision.allowed) return { response: null, user: context.user, context }; } return { response: admissionsJson({ error: "You do not have permission for this admissions action." }, 403), user: null, context: null }; }
