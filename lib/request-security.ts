...
export const COMMUNICATION_WEBHOOK_BODY_LIMIT_BYTES = 64 * 1024;
...
export function isProviderWebhookPath(pathname: string) {
  return pathname.startsWith("/api/whatsapp/webhook/") ||
    ...
    pathname.startsWith("/api/communication/webhook/");
}

export function requestBodyLimitBytes(pathname: string) {
  if (pathname.startsWith("/api/communication/webhook/")) return COMMUNICATION_WEBHOOK_BODY_LIMIT_BYTES;
