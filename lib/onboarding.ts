...
export async function createDryRunPlan(client: Client, batch: any, rows: OnboardingWorkbookRows, resolutions: Resolutions = {}) {
  ...
  for (const row of normalized.guardians) {
    ...
    if (existing) duplicateIssue(issues, "GUARDIAN_CONTACT_MATCH", "Guardians", row.rowNumber, row.rowKey, "Mobile", maskPhone(row.mobile), resolutions, "A possible existing Guardian uses this contact.");
  }
...
export async function rollbackOnboardingBatch(client: PrismaClient, publicKey: string, actor: IamActor, input: { reason: string; reauthPassword: string; execute?: boolean }) {
  ...
  const dependencies = await rollbackDependencies(client, ids, batch.executedAt);
...
function duplicateIssue(issues: OnboardingIssue[], code: string, sheet: string, row: number, rowKey: string, column: string, value: string, resolutions: Resolutions, message: string) { const decision = resolutions[rowKey]; if (!decision) issues.push({ code, severity: "REQUIRES_USER_DECISION", sheet, row, rowKey, column, submittedValue: value, message, suggestion: "Choose LINK_EXISTING, SKIP or REJECT_ROW and record a reason." }); else if (!decision.reason?.trim()) issues.push(issue("DECISION_REASON_REQUIRED", "BLOCKING_ERROR", sheet, row, rowKey, column, "A duplicate decision requires a reason.")); else if (decision.decision === "UPDATE_EXISTING") issues.push(issue("UPDATE_MODE_NOT_AUTHORISED", "BLOCKING_ERROR", sheet, row, rowKey, column, "Updates require a separately authorised correction workbook.")); else if (decision.decision === "CREATE_NEW") issues.push(issue("CREATE_DUPLICATE_REFUSED", "BLOCKING_ERROR", sheet, row, rowKey, column, "Create-new is not safe for an existing governed identifier; link, skip or reject the row.")); }
