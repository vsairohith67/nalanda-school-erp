...
export class ImportReceiptError extends Error {}
...
export function requireImportReceipt(token: unknown, context: unknown, now = Date.now()) {
  if (typeof token !== "string" || token.length > 1024) throw new ImportReceiptError("Validate this exact import again.");
  const [payload, signature, extra] = token.split(".");
  if (extra || !signature || !authSecretMatches(payload, "bulk-import-preview-v1", signature)) throw new ImportReceiptError("Import preview binding is invalid.");
  const parsed = JSON.parse(Buffer.from(payload, "base64url").toString("utf8"));
  if (parsed.expires < now || parsed.digest !== importDigest(context)) throw new ImportReceiptError("Import content, context or versions changed, or preview expired. Validate again.");
}
