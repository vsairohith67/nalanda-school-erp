...
type AssetManifest = {
  ...
  createdAt: string;
  assets: Array<{ publicKey: string; storageKey: string; mediaType: string; extension: string; byteSize: number; sha256: string; entry: string }>;
};
...
export async function createAndVerifyClassworkAssetBackup(client: PrismaClient, input: {
  ...
  const attachments = await client.classworkAttachment.findMany({
    where: input.attachmentPublicKeys?.length ? { publicKey: { in: input.attachmentPublicKeys } } : undefined,
  ...
  for (const attachment of attachments) {
    ...
    entries[entry] = bytes;
    manifest.assets.push({ publicKey: attachment.publicKey, storageKey: attachment.storageKey, mediaType: attachment.mediaType, extension: attachment.extension, byteSize: attachment.byteSize, sha256: attachment.sha256, entry });
  }
  ...
  if (first.manifestSha256 !== second.manifestSha256 || first.assetDigest !== second.assetDigest || first.assetCount !== attachments.length) throw new ClassworkAssetBackupError("The two isolated attachment restores do not match.");
...
function validateManifest(manifest: AssetManifest, archive: Record<string, Uint8Array>) {
  ...
  for (const row of manifest.assets) {
    if (!/^[A-Za-z0-9_-]{8,80}$/.test(row.publicKey) || publicKeys.has(row.publicKey) || !/^[a-f0-9]{2}\/[a-f0-9]{2}\/[a-f0-9-]{36}\.(?:pdf|png|jpg|webp)$/.test(row.entry) || row.entry !== row.storageKey || !/^[a-f0-9]{64}$/.test(row.sha256) || !Number.isSafeInteger(row.byteSize) || row.byteSize < 1) throw new ClassworkAssetBackupError("The encrypted classwork asset manifest contains an invalid entry.");
