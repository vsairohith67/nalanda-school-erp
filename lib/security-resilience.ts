...
export const RATE_LIMIT_POLICIES: readonly RateLimitPolicy[] = [
  ...
  policy("auth.access-invitation", "HIGH", ["POST"], 8, 15 * MINUTE, (path) => path.startsWith("/api/auth/invitations/") || path.startsWith("/api/auth/activation/")),
  policy("auth.mfa", "HIGH", ["POST", "DELETE"], 6, 10 * MINUTE, (path) => path.startsWith("/api/auth/mfa/") || path === "/api/auth/login/mfa" || path.startsWith("/api/auth/step-up/")),
  policy("iam.access-readiness", "HIGH", ["POST", "PATCH", "DELETE"], 10, 10 * MINUTE, (path) => path.startsWith("/api/user-access-readiness/")),
