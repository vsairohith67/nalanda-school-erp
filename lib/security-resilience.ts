...
export const RATE_LIMIT_POLICIES: readonly RateLimitPolicy[] = [
  ...
  policy("ocr.upload", "VERY_HIGH", ["POST"], 6, 10 * MINUTE, (path) => path === "/api/ocr/documents"),
  policy("ocr.mutation", "HIGH", ["POST", "PUT", "PATCH"], 30, MINUTE, (path) => path.startsWith("/api/ocr/")),
  policy("ocr.worker", "HIGH", ["POST", "PUT"], 240, MINUTE, (path) => path.startsWith("/api/internal/ocr/worker/")),
