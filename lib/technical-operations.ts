...
async function businessInvariantCounts(client: DatabaseClient) {
  const queries: Array<{ key: string; query: string }> = [
    ...
    { key: "migration_backup_mismatch", query: "SELECT COUNT(*) AS count FROM ReleaseManifest WHERE isCurrent=1 AND (backupVersion<>43 OR migrationVersion<>'20260822113000_event_media_v1_5_foundation')" }
  ];
