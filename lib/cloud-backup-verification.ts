...
export async function verifyStoredCloudBackupArtifact(
  ...
  try {
    ...
    if (!isSupportedStoredCloudBackupVersion(backup.metadata.backupVersion)) throw new Error("BACKUP_SCHEMA_INVALID");
    await record(prisma, artifact, "BACKUP_SCHEMA", "PASSED", `Nalanda backup schema and supported version ${backup.metadata.backupVersion} passed.`, Date.now() - started);
    ...
    return { verified: true, backupVersion: backup.metadata.backupVersion, artifactId: artifact.id };
  } catch (error) {
