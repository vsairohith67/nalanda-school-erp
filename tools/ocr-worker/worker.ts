...
async function processJob(job: ClaimedJob) {
  ...
  try {
    ...
    const modelMounts = ["--mount", `type=bind,src=${join(modelRoot, "official_models")},dst=/model-source,readonly`];
    const args = ["run", "--rm", "--name", containerName, "--gpus", "all", "--network", "none", "--read-only", "--cap-drop", "ALL", "--security-opt", "no-new-privileges", "--pids-limit", "512", "--memory", "24g", "--ulimit", "nofile=256:256", "--shm-size", "2g", "--tmpfs", "/tmp:rw,noexec,nosuid,nodev,size=512m", "--tmpfs", "/paddle-cache:rw,nosuid,nodev,size=64m", "--mount", `type=bind,src=${source},dst=/work/input/source${sourceExtension(job.sourceExtension)},readonly`, "--mount", `type=bind,src=${output},dst=/work/output`, ...modelMounts, "--env", "OCR_PADDLE_DEVICE=gpu:0", "--env", "OCR_TRUSTED_LAUNCHER=paddle-docker-network-none-v1", "--env", `OCR_PADDLE_IMAGE_ID=${imageId}`, "--env", "PADDLE_PDX_CACHE_HOME=/paddle-cache", "--env", "OCR_MODEL_SOURCE_ROOT=/model-source", "--env", "PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK=True", "--env", "HF_HUB_OFFLINE=1", "--env", "TRANSFORMERS_OFFLINE=1", imageId, "--source", `/work/input/source${sourceExtension(job.sourceExtension)}`, "--output", "/work/output", "--language", job.languageProfile];
