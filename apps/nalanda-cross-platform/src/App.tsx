  ...
  return (
    <div className="app-frame">
      <aside className="sidebar">
        <div className="brand"><img src="/nalanda-logo-transparent.png" alt="Nalanda Public School emblem" /><div><strong>Nalanda Public School</strong><span>Secure ERP workspace</span></div></div>
...
function LockScreen({ onUnlock }: { onUnlock: (pin: string) => Promise<void> }) {
  ...
  return <main className="lock-screen"><div className="lock-card"><img src="/nalanda-logo-transparent.png" alt="Nalanda Public School emblem" /><p className="eyebrow">SECURE ERP WORKSPACE</p><h1>Welcome back</h1><p>Unlock local encrypted drafts. This does not sign you into the school server.</p><label>App PIN<input type="password" inputMode="numeric" autoComplete="off" minLength={8} maxLength={12} value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, ""))} placeholder="8–12 digits" /></label>{error ? <p role="alert">{retryAfter > 0 ? `Too many failed attempts. Try again in ${retryAfter} seconds.` : error}</p> : null}<button className="primary" disabled={pin.length < 8 || busy || retryAfter > 0} onClick={() => void submit()}><LockKeyhole />{busy ? "Unlocking…" : retryAfter > 0 ? `Wait ${retryAfter}s` : "Unlock app"}</button><span className="reset-boundary">Forgot the PIN? An owner-authorized reset erases this app's encrypted local data and requires fresh device approval.</span></div></main>;
}
